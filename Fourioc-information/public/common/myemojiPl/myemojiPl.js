	...
	$.fn.myEmoji = function(options) {
		var defaults = {
			emojiconfig: {
				tieba: {
					name: "贴吧表情",
					path: "../common/myemojiPl/img/tieba/",
					maxNum: 50,
					file: ".jpg",
					placeholder: ":{alias}:",
					alias: {
			...
			function() {
				...
				if (emojiContainer.children().length <= 0) {
					...
					for (var emojilist in emojiconfig) {
						...
						for (var i = 1; i <= emojiconfig[emojilist].maxNum; i++) {
							if (emojiContainer.find('.for_' + emojilist) !== undefined) {
								...
								emojiContainer.find('.for_' + emojilist).append('<a href="#!" class="_img"><img class="_img" src="' + emojiconfig[emojilist].path + i + emojiconfig[emojilist].file + '" alt="" data-alias="'+(emojiconfig[emojilist].alias == undefined ? (i+emojiconfig[emojilist].file) : emojiconfig[emojilist].alias[i])+'" title="' + (emojiconfig[emojilist].title == undefined ? '' : emojiconfig[emojilist].title[i]) + '" /></a>');
							}
						}
					}
					...
					faceDiv.find('.emoji_container ._img').on('click', function() {
						...
						console.log('height:'+InputText.height());
						var aa = InputText.position();
						console.log('position:'+aa.left+' '+aa.top);
						if( InputText.height() == 33 ) {
							InputText[0].style.top -= 17;
						}
					});
